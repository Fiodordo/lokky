    ...
    <div className="lk-dashboard-home">
        ...
        <section className="lk-latest-card">
          ...
          <div className="lk-latest-metric">
            ...
            <div className="lk-score-ring" style={{ "--score-color": tone.color, "--score-angle": `${Math.max(tone.value, 8) * 3.6}deg` } as React.CSSProperties}>
              <strong>{latest.score}</strong><span>/100</span>
            </div>
          </div>
          ...
          <Link href={`/report/${latest.id}`} className="lk-report-btn">Voir le rapport <span>→</span></Link>
        </section>
      ) : (
      ...
      <div className="lk-content-grid">
        <section className="lk-panel">
          ...
          {loading ? <div className="lk-panel-empty">Chargement...</div> : scans.length === 0 ? <div className="lk-panel-empty">Aucun scan pour le moment.</div> : (
            <div className="lk-scan-table">
              {scans.slice(0, 5).map((scan, index) => {
                const t = scoreTone(scan.score);
                return (
                  <Link href={`/report/${scan.id}`} className="lk-scan-item" key={`${scan.id}-${index}`}>
        ...
        <section className="lk-panel lk-next-panel">
          ...
          <Link href="/dashboard/scanner" className="lk-next-item"><span className="lk-step-dot">1</span><span className="lk-step-copy"><strong>Scanner ton SaaS</strong><small>Découvre ton score et les risques détectés</small></span><span className="lk-step-action">Scanner →</span></Link>
          <Link href={latest ? `/report/${latest.id}` : "/dashboard/scanner"} className="lk-next-item"><span className="lk-step-dot">2</span><span className="lk-step-copy"><strong>Comprendre les problèmes</strong><small>{latest ? "Demande à Lokky comment les corriger" : "Ton assistant t'aidera après le scan"}</small></span><span className="lk-step-action">Parler à Lokky →</span></Link>
          <Link href={latest ? `/report/${latest.id}` : "/dashboard/scanner"} className="lk-next-item last"><span className="lk-step-dot">3</span><span className="lk-step-copy"><strong>Re-scanner après correction</strong><small>Vérifie que ton score s'améliore</small></span><span className="lk-step-action">Re-scanner →</span></Link>
        </section>
      </div>
      ...
      <section className="lk-maker-tip">
        ...
        <Link href={latest ? `/report/${latest.id}` : "/dashboard/scanner"} className="lk-tip-btn">Parler à Lokky →</Link>
      </section>
      ...
      <style jsx global>{`
        ...
        .lk-dashboard-head h1{font-size:38px;line-height:1.05;letter-spacing:-.045em;font-weight:650;margin:0 0 10px}.lk-dashboard-head h1 em{font-style:normal;color:#a855f7}.lk-dashboard-head p{font-size:14px;line-height:1.6;color:#858091;margin:0;max-width:680px}
        ...
        .lk-latest-card{min-height:165px;display:grid;grid-template-columns:minmax(260px,1.4fr) 180px 210px auto;align-items:center;gap:26px;padding:26px 28px;background:linear-gradient(115deg,rgba(139,92,246,.09),rgba(255,255,255,.018));border:1px solid rgba(139,92,246,.22);border-radius:16px;margin-bottom:24px}.lk-latest-domain{min-width:0}.lk-overline{display:block;font:600 10px ui-monospace,SFMono-Regular,Menlo,monospace;letter-spacing:.1em;color:#625d6e;text-transform:uppercase}.lk-latest-domain h2{font-size:25px;letter-spacing:-.035em;margin:10px 0 5px;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.lk-latest-domain p{font-size:12px;color:#6e687a;margin:0}.lk-external{font-size:15px;color:#777183;margin-left:5px}
