...
export default async function ReportPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="report-page">
    <header><Link href="/" className="logo"><span>◈</span> LOKKY</Link><Link href="/register" className="cta">Scanner mon SaaS →</Link></header>
    <div className="report-wrap">
      ...
      <section className={`verdict ${failed.length ? "danger" : "good"}`}><div className="verdict-icon">{failed.length ? "!" : "✓"}</div><div><b>{failed.length ? `${failed.length} point${failed.length > 1 ? "s" : ""} à corriger` : "Aucun problème détecté"}</b><p>{failed.length ? "Ton SaaS fonctionne, mais certains signaux de sécurité méritent ton attention avant de scaler." : "Les vérifications visibles depuis l'extérieur sont au vert."}</p></div></section>

      {failed.length > 0 && <section className="priority"><span className="eyebrow">À FAIRE EN PREMIER</span>{failed.slice(0,3).map((item,i)=><article key={item.label}><div className="num">{i+1}</div><div><h2>{item.icon} {item.label}</h2><p><strong>Pourquoi ça compte :</strong> {item.impact}.</p><div className="fix">→ Correction recommandée : consulte le guide Lokky et re-scanne après modification.</div></div></article>)}</section>}
